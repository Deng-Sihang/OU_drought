...
### 49-55 Listed as annotated information for species
comm = comm[,1:48]
dim(comm)
...
### It can also be used to calculate the temporal successions of plant richness, microbial biomass, relative abundance, and network topological properties. The results of permutation tests can be used depending on the situation.
treatused<-read.csv("Treatment info-Microbial biomass.csv",header = T,row.names = 1)
divindex<-read.csv("Bacteria-alphadiversity.csv",header = T,row.names = 1)
...
### Effect size
treatused<-read.csv("Treatment info-Microbial biomass.csv",header = T,row.names = 1)
divindex<-read.csv("Fungi-alphadiversity.csv",header = T,row.names = 1)
treatused$year<-treatused$year-2009  
...
### Taking total microbial biomass as an example (1-7 total microbial biomass, 8-14 total bacterial biomass, 15-21AM fungal biomass)
dat <- dat[,1:7] 
