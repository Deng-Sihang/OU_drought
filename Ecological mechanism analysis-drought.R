...
head(xms1)
xms1[,2:ncol(xms1)]<-scale(xms1[,2:ncol(xms1)])

### Take C fixation gene as an example
colnames(xms1)
be=xms1[,'Carbon.Fixation',drop=FALSE]
ym=as.matrix(be)
Yname=paste(colnames(ym))

colnames(xms1)
xms2=as.matrix(xms1[,c("Drought","moisture","pH","TOC","TN","NH4.N",
                       "NO3.N","FlTotl","FlC3","X16S","ITS","X18S",
                       "B.PC1","F.PC1","P.PC1","Carbon.degradation",
                       "Denitrification","Ammonification"), drop=FALSE])
head(xms2)
plstm=plsfw(xms2,ym,r2buf=0.98,Q2ck=FALSE,SEEck=FALSE,rand=100) 
