...
### Geochip analysis --------------------------------------------
treatused<-read.csv("Treatment info-Microbial biomass.csv",header = T,row.names = 1)
divindex<-read.csv("GeoChip-rawdata-2009-2014-Probe.csv",header = T,row.names = 1)

### 1 ### LMM on gene probe level
...
divindex<-read.csv("GeoChip-rawdata-2009-2014-Probe.csv",header = T,row.names = 1)
samp = match.name(rn.list = list(divs1 = divs1, divindex = divindex))
divindex = samp$divindex
divs1 <- cbind(divs1,divindex[,49:60])
write.csv(divs1,"GeoChip-rawdata-2009-2014-Probe-LMM.csv")
