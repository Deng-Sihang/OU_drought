...
com.file = "Bacteria-zotu-resampled.csv" #Bacteria,Fungi,Protists
...
nmds_dis_site$prefix <- "Bacteria" #Bacteria,Fungi,Protists,GeoChip
output <- rbind(output,nmds_dis_site)

result1 <- nmds_dis_site %>% group_by(Treatment, Year) %>%
  summarise(MDS1 = mean(MDS1), MDS2 = mean(MDS2), n = n(), .groups = "drop")
result1$prefix <- "Bacteria" #Bacteria,Fungi,Protists,GeoChip
result <- rbind(result,result1)

write.csv(result,"NMDS2.csv")
...
p.ds.perm =if (ds.obs < 0) {valid <- ds.ran < ds.obs
...
tdci$p.aic.perm = c(p.values[4,1],p.values[4,2])
...
p.beta.perm <- sum((if(beta.year < 0) betar$beta.year.r < 0 else betar$beta.year.r > 0) & 
                     ...
                     (if(betai < 0) betar$beta < betai else betar$beta > betai), na.rm = TRUE)/(nrow(betar)+1)
...
write.csv(output1,"AllbetaTaxa-LMM-TDR.csv")
